CREATE USER IF NOT EXISTS 'catalogue_user'@'%' IDENTIFIED BY 'default_password1';
CREATE USER IF NOT EXISTS 'catalogue_user'@'localhost' IDENTIFIED BY 'default_password1';

CREATE DATABASE socksdb;
USE socksdb;
GRANT ALL ON *.* TO 'catalogue_user'@'localhost';
GRANT ALL ON *.* TO 'catalogue_user'@'%' ;
alter user 'catalogue_user'@'%' identified with mysql_native_password by 'default_password';
alter user 'catalogue_user'@'localhost' identified with mysql_native_password by 'default_password';
FLUSH PRIVILEGES;
